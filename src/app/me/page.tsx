...
export default async function MePage() {
  ...
  const recentWishes = wishedProducts.slice(0, 4)
      ...
      <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col px-4 py-10">
        ...
        <section className="mt-10">
          <div className="flex items-baseline justify-between gap-3">
            ...
            <Link href="/me/orders" className="text-sm text-muted-foreground hover:underline">
              전체 보기
            </Link>
          </div>
          <OrderList orders={recentOrders} empty="아직 산 굿즈가 없어요." />
        </section>

        <section className="mt-10">
          <div className="flex items-baseline justify-between gap-3">
            <h2 className="font-heading text-lg font-semibold">찜한 굿즈</h2>
            <Link href="/me/wishlist" className="text-sm text-muted-foreground hover:underline">
              전체 보기
            </Link>
          </div>
