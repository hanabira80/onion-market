...
- [x] CSV에 있는 학번만 로그인된다
- [x] 첫 로그인 시 이름이 맞고, 비밀번호를 직접 정할 수 있다
- [x] 학번+이름으로 비밀번호를 다시 정할 수 있다
- [x] 학생이 상품을 보고 찜하고, 포인트가 충분하면 구매된다
- [x] 동시에 누르면 한 명만 사고 나머지는 품절이다
- [x] 수령 전 취소 시 포인트와 재고가 돌아온다
- [x] 마이페이지에서 포인트·구매·찜이 보인다
- [x] 관리자 학번만 상품 등록·포인트 지급(개별+일괄)을 할 수 있다
- [x] 관리자가 수령 완료로 바꾸면 학생은 취소할 수 없다
- [x] 회원가입 화면이 없다
...
1. `feat/p1-1-student-login` — CSV 명단 + 학번 로그인 + 비밀번호 찾기 ✅ (p1-3과 함께 메인 반영)
2. `feat/p1-2-admin-products` — 관리자 상품 업로드·포인트 지급 ✅ (p1-3과 함께 메인 반영)
3. `feat/p1-3-shop-catalog` — 목록·상세·찜·구매(선점) ✅
4. `feat/p1-4-mypage` — 마이페이지 + 수령 전 취소 ✅
5. `feat/p1-5-admin-orders` — 관리자 수령 완료 처리 ✅
